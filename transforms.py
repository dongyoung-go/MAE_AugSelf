...
class MultiView_oneside:
    def __init__(self, t1,t2, num_views=2):
        self.transform1 = t1
        self.transform2 = t2
        assert num_views==2
...
def extract_diff(transforms1, transforms2, crop1, crop2):
    ...
    # center2[f1, 1] = 1-center2[f1, 1] ## Error in official code
    center2[f2, 1] = 1-center2[f2, 1] 
    ...
    for t1, t2 in zip(transforms1, transforms2):
        if isinstance(t1, K.RandomHorizontalFlip):
            ...
            w1 = t1._params['batch_prob']
            w2 = t2._params['batch_prob']
            diff['gray'] = (w1==w2).float().unsqueeze(-1)

        elif isinstance(t1, GaussianBlur):
            # w1 = t1._params['batch_prob']
            # w2 = t2._params['batch_prob']
            # diff['gray'] = (w1==w2).float().unsqueeze(-1) # If we just compare applied or not
            w1 = _extract_w(t1)
            w2 = _extract_w(t2)
            diff['blur'] = w1-w2
...
def extract_each(transforms1, transforms2, crop1, crop2):
    each = {}
    for t1, t2 in zip(transforms1, transforms2):
        if isinstance(t1, K.RandomHorizontalFlip):
            f1 = t1._params['batch_prob']
    ...
    each['crop1'] = torch.cat([center1, crop1[:, 2:]], 1)
    each['crop2'] = torch.cat([center2, crop2[:, 2:]], 1)
    each['flip1'] = (f1).float().unsqueeze(-1)
    each['flip2'] = (f2).float().unsqueeze(-1)
    for t1, t2 in zip(transforms1, transforms2):
        if isinstance(t1, K.RandomHorizontalFlip):
            pass
